...
static BOOL FrostedGlassEnabled() {
    // PATCH: Enable frosted glass by default if the class exists
    // This matches YouTube's new UI design where all overlay buttons have semi-transparent backgrounds
    if (!hasFrostedGlass) return NO;

    // Check if the user has explicitly set a preference
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    id value = [defaults objectForKey:FrostedGlassBottomKey];

    // If no preference is set, default to YES (enabled)
    // If the user explicitly set it, respect their choice
    return (value == nil) ? YES : [defaults boolForKey:FrostedGlassBottomKey];
}
